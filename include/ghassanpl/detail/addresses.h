...
namespace ghassanpl::err
{
	...
	std::error_code make_error_code(uri_error_code);
	...
	template <typename T>
	using uri_expected = expected<T, std::error_code>;
	...
	struct known_uri_scheme
	{
		...
		virtual std::error_code validate(uri_view uri) const noexcept
		{
			auto decomposed = decompose_uri(uri);
			if (!decomposed) return decomposed.error();

			if (decomposed.value().scheme != scheme()) return make_error_code(uri_error_code::scheme_invalid);
		...
		virtual std::error_code validate_authority(std::string_view fragment) const noexcept
		{
		...
		virtual std::error_code validate_user_info(std::string_view element) const noexcept { return {}; }
		virtual std::error_code validate_host(std::string_view element) const noexcept { return {}; }
		virtual std::error_code validate_port(std::string_view element) const noexcept { return {}; }
		virtual std::error_code validate_path(std::string_view element) const noexcept { return {}; }
		virtual std::error_code validate_query(std::string_view element) const noexcept { return {}; }
		virtual std::error_code validate_fragment(std::string_view element) const noexcept { return {}; }
		...
		virtual std::error_code iterate_scheme_elements(uri_view uri, std::function<bool(std::string_view element_name, std::string_view element_value)> callback) const noexcept
		{
			return make_error_code(uri_error_code::no_scheme_specific_elements);
		}
...
{
	template <>
	struct is_error_code_enum<ghassanpl::err::uri_error_code> : true_type {};
}
