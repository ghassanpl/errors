#include "../../include/ghassanpl/detail/addresses.h"
...
{
	struct uri_error_category : std::error_category
	{
		virtual const char* name() const noexcept override { return "uri_category"; }

		virtual std::string message(int err) const override { return std::string{ magic_enum::enum_name(uri_error_code(err)) }; }
	};

	std::error_category const& uri_category()
	{
		static uri_error_category cat;
		return cat;
	}

	std::error_code make_error_code(uri_error_code code)
	{
		return std::error_code{ (int)code, uri_category() };
	}
	...
	void split_query_elements(std::string_view path, std::vector<std::pair<std::string, std::string>>& elements)
	{
		split(path, QUERY_DELIMITER, [&elements](std::string_view el, bool last) {
			auto k = consume_until(el, KEY_DELIMITER); trim(el);
	...
	uri_expected<decomposed_uri> decompose_uri(uri_view uri, enum_flags<uri_decompose_flags> const flags)
	{
		try
		{
			...
			result.scheme = parse_scheme(uri, flags);
			...
			if (consume(uri, "//"))
			{
				result.authority = parse_authority(uri, flags);

				auto [user_info, host, port] = parse_authority_elements(result.authority, flags);
				if (port.empty() && flags.contain(uri_decompose_flags::use_well_known_port_numbers))
					port = deduce_port_from_scheme(result.scheme);
				result.user_info = std::move(user_info);
				result.host = std::move(host);
				result.port = std::move(port);
			}

			auto [path, elements] = parse_path(!result.authority.empty(), uri, flags);
			...
			if (consume(uri, '?'))
			{
				auto [query, elements] = parse_query(uri, flags);
				result.query = std::move(query);
				result.query_elements = std::move(elements);
			}

			if (consume(uri, '#'))
				result.fragment = parse_fragment(uri, flags);
	...
	{
		return
			ascii::strings_equal_ignore_case(scheme, other.scheme) &&
			user_info == other.user_info &&
			ascii::strings_equal_ignore_case(host, other.host) &&
			trimmed(port, '0') == trimmed(other.port, '0') &&
	...
	namespace known_schemes
	{
		...
		struct url_schemes : known_uri_scheme
		{
			...
			virtual std::error_code validate_host(std::string_view element) const noexcept override { if (element.empty()) return make_error_code(uri_error_code::host_required_in_scheme); return {}; }
		};

		struct http_schemes : url_schemes
		{
			virtual std::error_code validate_user_info(std::string_view element) const noexcept override { return {}; }
			virtual std::error_code validate_path(std::string_view element) const noexcept override { if (!element.empty() || element.starts_with("/")) return make_error_code(uri_error_code::path_malformed); return {}; }

			virtual std::string normalize_port(std::string_view element) const noexcept override { if (trimmed(element, '0') == default_port()) return {}; return std::string{ element }; }
			...
			virtual std::string normalize_host(std::string_view element) const noexcept { return ascii::tolower(element); }
	...
	std::vector<std::pair<std::string, std::string>> known_uri_scheme::split_query_elements(std::string_view query) const noexcept
	{
		...
		err::split_query_elements(query, result);
		return result;
	}
}